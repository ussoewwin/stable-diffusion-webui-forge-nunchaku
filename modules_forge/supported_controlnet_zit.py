...
class ZImageControlPatch:
    def __init__(self, model_patch, vae, image, strength, inpaint_image=None, mask=None):
        # CRITICAL: Verify model_patch.model is ZImage_Control, not ModelPatcher
        # This ensures we're calling the actual model, not a wrapper
        if not hasattr(model_patch, 'model'):
            raise ValueError(f"model_patch must have 'model' attribute, got {type(model_patch)}")
        if not isinstance(model_patch.model, comfy.ldm.lumina.controlnet.ZImage_Control):
            raise ValueError(f"model_patch.model must be ZImage_Control, got {type(model_patch.model)}")
    ...
    def __call__(self, kwargs):
        ...
        # CRITICAL: Store ZImage_Control model reference locally to ensure direct call
        # This prevents any potential ModelPatcher wrapper or transformer_options from interfering
        zimage_control_model = self.model_patch.model
        if not isinstance(zimage_control_model, comfy.ldm.lumina.controlnet.ZImage_Control):
            raise TypeError(f"Expected ZImage_Control, got {type(zimage_control_model)}")
        
        if self.temp_data is None or self.temp_data[0] > cnet_index:
            if block_type == "noise_refiner":
                # Direct call to ZImage_Control.forward, no transformer_options
                self.temp_data = (-3, (None, zimage_control_model(txt, self.encoded_image.to(img.dtype), pe, vec)))
            else:
                # Direct call to ZImage_Control.forward, no transformer_options
                self.temp_data = (-1, (None, zimage_control_model(txt, self.encoded_image.to(img.dtype), pe, vec)))

        if block_type == "noise_refiner":
            next_layer = self.temp_data[0] + 1
            # Direct call to ZImage_Control method, no transformer_options
            self.temp_data = (next_layer, zimage_control_model.forward_noise_refiner_block(block_index, self.temp_data[1][1], img_input[:, :self.temp_data[1][1].shape[1]], None, pe, vec))
            ...
            while self.temp_data[0] < cnet_index and (self.temp_data[0] + 1) < cnet_blocks:
                next_layer = self.temp_data[0] + 1
                # Direct call to ZImage_Control method, no transformer_options
                self.temp_data = (next_layer, zimage_control_model.forward_control_block(next_layer, self.temp_data[1][1], img_input[:, :self.temp_data[1][1].shape[1]], None, pe, vec))
...
class ZImageControlNetPatcher(ControlModelPatcher):
    ...
    def process_before_every_sampling(self, process, cond, mask, *args, **kwargs):
        ...
        vae_raw = process.sd_model.forge_objects.vae
        if not hasattr(vae_raw, "spacial_compression_encode"):
            vae = ZImageVAEWrapper(vae_raw)
        else:
            vae = vae_raw
        ...
        # CRITICAL: Verify model_patcher is ComfyUI ModelPatcher and model is ZImage_Control
        # This ensures ZImageControlPatch receives the correct model_patch
        if not isinstance(self.model_patcher, comfy.model_patcher.ModelPatcher):
            raise TypeError(f"self.model_patcher must be ComfyUI ModelPatcher, got {type(self.model_patcher)}")
        if not isinstance(self.model_patcher.model, comfy.ldm.lumina.controlnet.ZImage_Control):
            raise TypeError(f"self.model_patcher.model must be ZImage_Control, got {type(self.model_patcher.model)}")
        
        if isinstance(self.model_patcher.model, comfy.ldm.lumina.controlnet.ZImage_Control):
            patch = ZImageControlPatch(self.model_patcher, vae, image, self.strength, inpaint_image=inpaint_image, mask=mask)
