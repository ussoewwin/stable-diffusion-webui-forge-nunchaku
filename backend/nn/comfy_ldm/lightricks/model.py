...
def precompute_freqs_cis(
    ...
    split_positional_embedding=LTXRopeType.INTERLEAVED,
    freq_grid_generator=None,
):
    """
    Module-level function for computing frequency-based positional embeddings (RoPE).
    This is a compatibility function for legacy code that expects a module-level function.
    ...
    Args:
        ...
        max_pos: Maximum positions for each dimension
        use_middle_indices_grid: Whether to use middle indices grid
        num_attention_heads: Number of attention heads
        split_positional_embedding: Type of positional embedding (INTERLEAVED or SPLIT)
        freq_grid_generator: Function to generate frequency grid (None for default)
    
    Returns:
        Tuple of (cos_freq, sin_freq, split_mode)
    """
    if freq_grid_generator is None:
        freq_grid_generator = generate_freq_grid_pytorch
    
    split_mode = split_positional_embedding == LTXRopeType.SPLIT
