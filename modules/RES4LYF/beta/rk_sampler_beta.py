...
from comfy.model_sampling import EPS, CONST
import comfy

# Import model types for detection
try:
    from backend.diffusion_engine.qwen import QwenImage as ForgeQwenImage
    from backend.diffusion_engine.flux import Flux as ForgeFlux
    FORGE_MODELS_AVAILABLE = True
except ImportError:
    FORGE_MODELS_AVAILABLE = False
...
def sample_rk_beta(
    ...
    # Get device from model - for Nunchaku QwenImage models, device attribute may not exist
    # Try multiple methods: model attribute, model parameters, or x.device as fallback
    try:
        model_device = model.inner_model.inner_model.device
    except (AttributeError, TypeError):
        # Try getting device from model parameters (works for most PyTorch models)
        try:
            inner_model = model.inner_model.inner_model
            if hasattr(inner_model, 'parameters'):
                model_device = next(iter(inner_model.parameters())).device
            else:
                # Fallback to x.device (used in ComfyUI-nunchaku)
                model_device = x.device
        except (StopIteration, AttributeError, TypeError):
            # Final fallback: use x.device (safe and always available)
            model_device = x.device
    ...
    # Get model_sampling - different paths for Forge vs ComfyUI
    # ComfyUI BaseModel: model.inner_model.inner_model.model_sampling (direct attribute)
    # Forge ForgeDiffusionEngine: model.inner_model.inner_model.forge_objects.unet.model.model_sampling (via KModel)
    ...
    if hasattr(model, "inner_model") and hasattr(model.inner_model, "inner_model"):
        inner_model = model.inner_model.inner_model
        # Check if it's Forge model (has forge_objects attribute)
        if hasattr(inner_model, "forge_objects"):
            try:
                forge_objects = inner_model.forge_objects
                if hasattr(forge_objects, "unet") and hasattr(forge_objects.unet, "model"):
                    # Check diffusion_model type to distinguish Nunchaku models
    ...
    if model_sampling_obj is None:
        try:
            model_sampling_obj = model.inner_model.inner_model.model_sampling
    ...
    if model_sampling_obj is None:
        ...
        try:
            model_sampling_obj = model.inner_model.model_sampling
    ...
    if model_sampling_obj is None:
        ...
        try:
            model_sampling_obj = model.model_sampling
    ...
    # Ensure model_options and transformer_options exist
    if 'model_options' not in RK.extra_args:
        RK.extra_args['model_options'] = {}
    if 'transformer_options' not in RK.extra_args['model_options']:
        RK.extra_args['model_options']['transformer_options'] = {}
    ...
    if EO("y0_inv_to_transformer_options"):
        ...
        # Only for SD1.x/2.x models with diffusion_model (not Qwen Image)
        inner_inner = None
        try:
            if hasattr(model, "inner_model") and hasattr(model.inner_model, "inner_model"):
                inner_inner = model.inner_model.inner_model
        except (AttributeError, TypeError):
            pass
        
        if inner_inner is not None and hasattr(inner_inner, "diffusion_model"):
            diffusion_model = inner_inner.diffusion_model
    ...
    if guides is not None:
        ...
        if StyleMMDiT is not None:
            ...
            if EO("mycoshock"):
                ...
                if inner_inner is not None and hasattr(inner_inner, "diffusion_model") and hasattr(inner_inner.diffusion_model, "Retrojector"):
                    StyleMMDiT.Retrojector = inner_inner.diffusion_model.Retrojector
                    image_initial_shock = StyleMMDiT.apply_data_shock(image_initial.to(x))
                else:
                    image_initial_shock = image_initial.to(x)
                if VE_MODEL:
                    x = image_initial_shock.to(x) + sigmas[0] * noise_initial.to(x)
                else:
                    x = (1 - sigmas[0]) * image_initial_shock.to(x) + sigmas[0] * noise_initial.to(x)

    RK.update_transformer_options({"model_sampling": model_sampling_obj})
    # BEGIN SAMPLING LOOP
                    ...
                    if row < RK.rows:
                        ...
                        if RK.IMPLICIT   and   row == 0   and   (EO("implicit_lazy_recycle_first_model_call_at_start")   or   EO("radaucycle")  or RK.C[0] == 0.0):
                                    ...
                                    if noise_bongflow is None:
                                        ...
                                        if inner_inner is not None and hasattr(inner_inner, "diffusion_model"):
                                            ...
                                            if RK.extra_args['model_options']['transformer_options'].get('y0_standard_guide') is not None:
                                                if hasattr(diffusion_model, "y0_standard_guide"):
                                                    LG.y0 = y0_standard_guide = diffusion_model.y0_standard_guide.clone()
                                                    del diffusion_model.y0_standard_guide
                                            ...
                                            if RK.extra_args['model_options']['transformer_options'].get('y0_inv_standard_guide') is not None:
                                                if hasattr(diffusion_model, "y0_inv_standard_guide"):
                                                    LG.y0_inv = y0_inv_standard_guide = diffusion_model.y0_inv_standard_guide.clone()
                                                    del diffusion_model.y0_inv_standard_guide
                            ...
                            if    SYNC_GUIDE_ACTIVE:
                                ...
                                # Check for fp64 model out override (for testing only) - only for SD1.x/2.x models with diffusion_model
                                inner_inner = None
                                try:
                                    if hasattr(model, "inner_model") and hasattr(model.inner_model, "inner_model"):
                                        inner_inner = model.inner_model.inner_model
                                ...
                                if inner_inner is not None and hasattr(inner_inner, "diffusion_model") and hasattr(inner_inner.diffusion_model, "eps_out"):
                                    eps_out = inner_inner.diffusion_model.eps_out
                                    del inner_inner.diffusion_model.eps_out
                            ...
                            if inner_inner is not None and hasattr(inner_inner, "diffusion_model"):
                                ...
                                if RK.extra_args['model_options']['transformer_options'].get('y0_standard_guide') is not None:
                                    if hasattr(diffusion_model, "y0_standard_guide"):
                                        LG.y0 = diffusion_model.y0_standard_guide.clone()
                                        del diffusion_model.y0_standard_guide
                                        RK.extra_args['model_options']['transformer_options']['y0_standard_guide'] = None
                                
                                if RK.extra_args['model_options']['transformer_options'].get('y0_inv_standard_guide') is not None:
                                    if hasattr(diffusion_model, "y0_inv_standard_guide"):
                                        LG.y0_inv = diffusion_model.y0_inv_standard_guide.clone()
                                        del diffusion_model.y0_inv_standard_guide
                                        RK.extra_args['model_options']['transformer_options']['y0_inv_standard_guide'] = None
    ...
    if step == len(sigmas)-2 and sigmas[-1] == 0 and sigmas[-2] == NS.sigma_min and not INIT_SAMPLE_LOOP:
        if EO("skip_final_model_call"):
            ...
            denoised  = model_sampling_obj.calculate_denoised(sigma_min, eps, x)
            x = denoised
        else:
