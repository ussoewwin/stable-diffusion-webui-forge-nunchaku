...
class RK_NoiseSampler:
    def __init__(self,
        ...
        if model_sampling is None:
            ...
            if has_nested_attr(model, "model.model_sampling"):
        ...
        # Check if CONST type - check both comfy.model_sampling.CONST and backend.modules.k_model.CONST
        # ModelSamplingFluxWithConst uses backend.modules.k_model.CONST
        try:
            from backend.modules.k_model import CONST as BackendCONST
        except ImportError:
            BackendCONST = None
        
        self.CONST = isinstance(model_sampling, comfy.model_sampling.CONST) or (BackendCONST is not None and isinstance(model_sampling, BackendCONST))
